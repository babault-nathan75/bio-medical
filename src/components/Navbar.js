...
export default function Navbar({ backLink = "/" }) {
  ...
  // Fonction déclenchée à la soumission (Touche Entrée ou clic sur la loupe)
  const handleSearch = (e) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      router.push(`/shop?search=${encodeURIComponent(searchQuery.trim())}`);
    } else {
      router.push('/shop'); // Si le champ est vide, on affiche tout
    }
  };
  ...
  return (
    <nav className="bg-white/80 backdrop-blur-md border-b border-gray-100 px-6 py-4 sticky top-0 z-50 flex justify-between items-center shadow-sm">
      ...
      <div className="flex items-center gap-4">
        {/* NOUVEAU : Le formulaire de recherche */}
        <form onSubmit={handleSearch} className="relative hidden md:block">
          ...
          />
          <button type="submit" className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#B57C4F] transition-colors">
